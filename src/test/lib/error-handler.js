...
describe('/lib/error-handler.js', () => {
  it('exports an object with function fields', () => {
    ...
    expect(errorHandler).to.have
      .property('videoPageError')
      .which.is.a('function')
    expect(errorHandler).to.have
      .property('noCommentsError')
      .which.is.a('function')
  ...
  it(`videoPageError returns a ${errorTypes.VIDEO_ERROR} object if no html is given`, () => {
    ...
    expect(
      errorHandler.videoPageError({ component, operation, videoId })
    ).to.deep.equal({
      component,
      operation,
      videoId,
  ...
  it(`videoPageError detects ${errorTypes.VIDEO_ERROR_COUNTRY_RESTRICTION} errors`, () => {
    ...
    expect(
      errorHandler.videoPageError({ component, operation, videoId, html })
    ).to.deep.equal({
      component,
      operation,
      videoId,
  ...
  it(`videoPageError detects ${errorTypes.VIDEO_ERROR_UNAVAILABLE} errors`, () => {
    ...
    const videoId = 'test-video'
    const message =
      'This video is no longer available due to a copyright claim by Your Mama.'
    const html = `
      <div>
        <div id="player-unavailable">
          <div>
  ...
  it(`noCommentsError returns a ${errorTypes.VIDEO_ERROR_NO_COMMENTS} error`, () => {
    ...
    expect(
      errorHandler.noCommentsError({ component, operation, videoId })
    ).to.deep.equal({
      component,
      operation,
      videoId,
  ...
  it(`scraperError returns a ${errorTypes.SCRAPER_ERROR} error`, () => {
    ...
    expect(
      errorHandler.scraperError({ component, operation, videoId, message })
    ).to.deep.equal({
      component,
      operation,
      videoId,
