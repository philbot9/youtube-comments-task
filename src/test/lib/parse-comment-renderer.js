...
describe('/lib/parse-comment-renderer.js', () => {
  ...
  it('parses simple comment fields', done => {
    ...
    const $commentRenderer = cheerio.load(html)(
      '.comment-thread-renderer > .comment-renderer:nth-child(1)'
    )

    parseCommentRenderer($commentRenderer).fold(
      e => {
        expect.fail(e)
        done(e)
      },
  ...
  it('handles missing fields on comment', done => {
    ...
    $commentRenderer
      .find(
        '.comment-renderer-content > .comment-renderer-text > .comment-renderer-text-content'
      )
      .remove()
    $commentRenderer
      .find('.comment-action-buttons-toolbar > .comment-renderer-like-count.on')
      .remove()
    $commentRenderer
      .find('.comment-renderer-header > .comment-renderer-time')
      .remove()
