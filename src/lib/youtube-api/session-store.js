...
const extractToken = (html, regex) => Either.fromNullable(regex.exec(html))
  .chain(m => Either.fromNullable(m[1]))
  .map(token => decodeURIComponent(token))

const extractSessionToken = html => extractToken(html, /'XSRF_TOKEN'\s*\n*:\s*\n*"(.*)"/i)
  .leftMap(_ => videoPageError({
    component: 'session-store',
    operation: 'extractSessionToken',
  html}))

const extractCommentsToken = html => extractToken(html, /'COMMENTS_TOKEN'\s*\n*:\s*\n*"([^"]+)"/i)
  .leftMap(_ => videoPageError({
    component: 'session-store',
...
const buildSession = (sessionToken, commentsToken) => ({sessionToken, commentsToken})

const getSession = videoId => fetchVideoPageWithRetries(videoId)
  .chain(html => liftM2(
    buildSession,
