...
const hasReplies = $commentThread =>
  cheerioFind($commentThread, '.comment-replies-renderer > div').fold(
    _ => false,
    _ => true
  )

const areRepliesCollapsed = $repliesContainer =>
  cheerioFind($repliesContainer, '.yt-uix-expander-collapsed').fold(
    _ => false,
    _ => true
  )
...
const parseCommentReplies = (comment, $repliesRenderer) =>
  parseReplies($repliesRenderer).map(replies =>
...
const addReplies = (comment, $commentThread) =>
  getRepliesRenderer($commentThread).chain(
    $repliesRenderer =>
