...
const {cheerioAttr, cheerioFindText, cheerioFindAttr} = require('./utils/cheerio-utils')

const resolveAny = (e, def) => e.fold(_ => def, x => x)

const compact = pickBy(v => v != null && v !== '')

const extractLikes = $c => cheerioFindText($c, '.comment-action-buttons-toolbar > .comment-renderer-like-count.on')
  .chain(strToInt)
  .map(l => l - 1)

const parseFromNow = fromNow => regExec(/(\d+)\s(\w+)\sago/, fromNow)
  .map(([_, n, u]) => moment().subtract(n, u).valueOf())

const commentEdited = $c => cheerioFindText($c, '.comment-renderer-header > .comment-renderer-time')
  .map(t => /\(edited\)/i.test(t))

const extractTime = $c => cheerioFindText($c, '.comment-renderer-header > .comment-renderer-time')
  .map(t => t.replace(/\s*\(edited\)/i, ''))
  .map(strTrim)

const addTimestamp = comment => Either.fromNullable(comment.time)
  .chain(parseFromNow)
...
const parseCommentRenderer = $commentRenderer => Either.fromNullable($commentRenderer)
