...
const extractLikes = $c =>
  cheerioFindText(
    $c,
    '.comment-action-buttons-toolbar > .comment-renderer-like-count.on'
  )
    .chain(strToInt)
    .map(l => l - 1)

const parseFromNow = fromNow =>
  regExec(/(\d+)\s(\w+)\sago/, fromNow).map(([_, n, u]) =>
    moment().subtract(n, u).valueOf()
  )
...
const extractFields = $c => ({
  ...
  authorLink: resolveAny(
    cheerioFindAttr($c, 'a.comment-author-text', 'href').map(strTrim)
  ),
  authorThumb: resolveAny(
