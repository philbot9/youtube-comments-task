...
const getContentHtml = response => Either.fromNullable(prop(response, 'content_html'))
  .leftMap(_ => 'API response does not contain a "content_html" field')

const extractNextPageToken = loadMoreWidgetHtml => regExec(/data-uix-load-more-post-body\s*=\s*"page_token=([^"]+)"/i, loadMoreWidgetHtml)
  .map(([_, t]) => t)
  .map(strTrim)
  .map(decodeURIComponent)
  .leftMap(_ => 'API resonse load_more_widget_html does not contain a next page token')

const fetchCommentPage = (videoId, pageToken) => commentPage(videoId, pageToken)
  .map(p => liftM2(
